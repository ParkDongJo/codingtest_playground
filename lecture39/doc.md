...
- 입력값으로 들어온 문자열을 반복문으로 첫글자만 모두 확인한다.
- map 에 key는 성, count 는 갯수로 해서 채워넣는다.
- map 을 count 기준으로 내림차순으로 정렬한다.
- count 가 5인 key를 하나씩 뽑아내서 배열에 넣는다
- key 를 넣은 배열을 오름차순으로 정렬한다.
- 만약 빈배열이면 PREDAJA를 뱉는다.
- 최종 값을 뱉는다.
